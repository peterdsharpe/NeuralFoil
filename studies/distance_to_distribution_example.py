...
cov_principle_components = np.array([vector_2d(3, 45), vector_2d(0.3, -45)]).T
...
n = 10000
data = np.random.multivariate_normal(mean=[1, 1], cov=cov, size=n)
...
fig, ax = plt.subplots()
x, y = data.T
p.sns.scatterplot(x=x, y=y, s=5, color="black", alpha=0.5)
...
mahalanobis_distance_squared = np.sum(
    (point - mean) @ np.linalg.inv(cov) * (point - mean), axis=1
)
...
p.contour(
    X,
    Y,
    np.sqrt(mahalanobis_distance_squared.reshape(X.shape)),
    levels=np.arange(10),
    alpha=0.5,
)
p.show_plot()
