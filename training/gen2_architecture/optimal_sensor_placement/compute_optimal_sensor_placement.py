...
for i in sensor_ids:
    if i < Data.N:
        ...
        plt.plot(*af.upper_coordinates(x).T, ".k", markersize=9, markeredgewidth=0, alpha=0.9)
    else:
        x = np.linspace(0, 1, Data.N)[i - Data.N]
        plt.plot(*af.lower_coordinates(x).T, ".k", markersize=9, markeredgewidth=0, alpha=0.9)
