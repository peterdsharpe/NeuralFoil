...
def model(x, p):
    ...
    thickness_modes = [
        x[f"kulfan_upper_{i}"] - x[f"kulfan_lower_{i}"] for i in range(8)
    ]
    camber_modes = [x[f"kulfan_upper_{i}"] + x[f"kulfan_lower_{i}"] for i in range(8)]

    CL0_high = (
        sum([p[f"CL0_high_{i}"] * camber_modes[i] for i in range(8)])
        + p["CL0_high_LE"] * x["kulfan_LE_weight"]
    )
    CL0_low = (
        sum([p[f"CL0_low_{i}"] * camber_modes[i] for i in range(8)])
    ...
    CL0 = np.blend(switch, CL0_high, CL0_low)
    CLa = np.blend(switch, CLa_high, CLa_low)
...
x_data = {k: v.to_numpy() for k, v in df_train[input_cols].to_dict().items()}
...
if __name__ == "__main__":
    ...
    y_test_model = fit(
        {k: v.to_numpy() for k, v in df_test[input_cols].to_dict().items()}
    )
    ...
    for Re, color in tqdm(
        zip(Res, colors),
        desc="Sweeping Reynolds Numbers...",
        total=len(Res),
        unit="runs",
    ):
        try:
        ...
        fit_aero = fit(
            {
                ...
                "Re": Re,
                **{
                    f"kulfan_lower_{i}": kulfan_params["lower_weights"][i]
                    for i in range(8)
                },
                **{
                    f"kulfan_upper_{i}": kulfan_params["upper_weights"][i]
                    for i in range(8)
                },
        ...
        plt.plot(alpha_plot, fit_aero, ":", color=color, alpha=0.6)
    ...
    plt.legend(title="Reynolds Numbers", fontsize=10, ncols=2, loc="lower right")

    axaf = ax.inset_axes(bounds=(0.05, 0.7, 0.25, 0.25))
    axaf.plot(af.x(), af.y(), "-k", alpha=0.8, zorder=100)
