...
af = af.translate(translate_y=-af.local_camber(x_over_c=0.5)).rotate(
    angle=np.radians(-6), x_center=0.5
)

### Compute where the Neural Network nodes should be drawn, and the directions to propagate them in
nodes_start = np.concatenate(
    [af.upper_coordinates()[::-1], af.lower_coordinates()[::-1]], axis=0
)
nodes_start_tangent_direction = np.roll(nodes_start, -1, axis=0) - np.roll(
    nodes_start, 1, axis=0
)
nodes_start_normal = (
    np.rotation_matrix_2D(np.pi / 2) @ (nodes_start_tangent_direction.T)
).T
nodes_start_direction = nodes_start_normal / np.linalg.norm(
    nodes_start_normal, axis=1
).reshape(-1, 1)
...
nodes_end = np.stack([0.6 + 1 * np.cosd(theta_deg), 0 + 1 * np.sind(theta_deg)], axis=1)
nodes_end_direction = np.stack([np.ones(n_nodes), np.zeros(n_nodes)], axis=1)

from scipy import interpolate


nodes_start_direction = np.stack(
    [np.zeros(n_nodes), np.where(np.arange(n_nodes) < n_nodes_per_side, 1, -1)], axis=1
)
...
for i, (layer_nodes, layer_color) in enumerate(zip(all_nodes, layer_colors)):
    ...
    if i <= np.argmin(all_nodes[:, :, 0].min(axis=1)):
        layer_nodes_for_ring = np.append(
            layer_nodes_for_ring, layer_nodes_for_ring[0, :].reshape(1, -1), axis=0
        )
    ...
    if i != len(all_nodes) - 1:
        xn = all_nodes[i + 1][
            :, 0
        ]  # The x-coordinates of the nodes in the (n)ext layer
        yn = all_nodes[i + 1][
            :, 1
        ]  # The y-coordinates of the nodes in the (n)ext layer

        for neighbor_offset in range(
            -n_neighbors_to_connect, n_neighbors_to_connect + 1
        ):
            ...
            ax.plot(
                np.stack(
                    [
                        roll_truncate(x, -neighbor_offset),
                        roll_truncate(xn, neighbor_offset),
                    ]
                ),
                np.stack(
                    [
                        roll_truncate(y, -neighbor_offset),
                        roll_truncate(yn, neighbor_offset),
                    ]
                ),
                color=layer_color,
                linewidth=0.5,
...
plt.text(
    ...
    ha="right",
    va="bottom",
    color=p.adjust_lightness(colors[0], 0.75),
    **text_kwargs
)
plt.text(
    x_offset,
    ...
    ha="right",
    va="top",
    color=p.adjust_lightness(colors[-1], 0.75),
    **text_kwargs
)
